...
% graduate setup
\documentclass[master, fontset=none]{shtthesis}
\shtsetup{
  degree-name = {工学硕士},
...
\def\chsteststr{永和九年，岁在癸丑，暮春之初，会于会稽山阴之兰亭，修禊事也。群贤毕至，少长咸集。此地有崇山峻岭，茂林修竹；又有清流激湍，映带左右，引以为流觞曲水，列坐其次。虽无丝竹管弦之盛，一觞一咏，亦足以畅叙幽情。}
\ExplSyntaxOn
\ProvideDocumentCommand{\chsfonttest}{%
    O{Auto} % bold font name
    m % font fullname
    m}{ % font name printed
  \IfFontExistsTF{#2}{%
    \let\testfont\relax%
    \ifthenelse{\equal{#1}{Auto}}{%
      \newCJKfontfamily\testfont{#2}[AutoFakeBold=2.5]%
    }{%
      \newCJKfontfamily\testfont{#2}[BoldFont=#1]%
    }%
    \begingroup%
      \zihao{5}\testfont\par\textbf{#3}\hspace*{1\ccwd}\chsteststr%
    \endgroup%
  }{}
}
\ExplSyntaxOff
...
\subsection{宋体}
\rmfamily\upshape
\chsfonttest{SimSun}{中易宋体}
\chsfonttest[Songti SC Bold]{Songti SC Light}{华文宋体}
\chsfonttest{FZShuSong-Z01}{方正书宋}
\chsfonttest[Noto Serif CJK SC Medium]{Noto Serif CJK SC Light}{思源宋体}
\chsfonttest[FandolSong-Bold.otf]{FandolSong-Regular.otf}{Fandol宋体}

\subsection{黑体}
\sffamily\upshape
\chsfonttest{SimHei}{中易黑体}
\chsfonttest[Heiti SC Medium]{Heiti SC Light}{华文黑体}
\chsfonttest{FZHei-B01}{方正黑体}
\chsfonttest[* Medium]{Noto Sans CJK SC}{思源黑体}
\chsfonttest[FandolHei-Bold.otf]{FandolHei-Regular.otf}{Fandol黑体}

\subsection{楷体}
\rmfamily\itshape
\chsfonttest{KaiTi}{中易楷体}
\chsfonttest[Kaiti SC Bold]{Kaiti SC Regular}{华文楷体}
\chsfonttest{FZKai-Z03}{方正楷体}
\chsfonttest{FandolKai-Regular.otf}{Fandol楷体}

\subsection{仿宋}
\ttfamily\upshape
\chsfonttest{FangSong}{中易仿宋}
\chsfonttest{STFangsong}{华文仿宋}
\chsfonttest{FZFangSong-Z02}{方正仿宋}
\chsfonttest{FandolFang-Regular.otf}{Fandol仿宋}

% reset Latin font shapes
\rmfamily\upshape
\fi
...
\def\engteststr{The quick brown fox jumps over the lazy dog.}
\ExplSyntaxOn
% TODO: remove redudant code here
\ProvideDocumentCommand{\engfonttest}{%
    o % bold font name
    m % full font name
    m}{% printed font name
  \IfFontExistsTF{#2}{%
    \let\testfont\relax%
    \IfNoValueTF{#1}{%
      \newfontfamily\testfont{#2}%
    }{%
      \newfontfamily\testfont{#2}[BoldFont=#1]%
    }%
    \testfont\textbf{#3} &%
    \let\testfont\relax%
    \IfNoValueTF{#1}{%
...
\begin{tabular}{ll}
  \toprule
  \engfonttest{Times New Roman}{Times New Roman}
  \engfonttest{Times}{Times}
  \engfonttest[XITS-Bold.otf]{XITS-Regular.otf}{XITS}
  \engfonttest[STIX-Bold.otf]{STIX-Regular.otf}{STIX}
  \engfonttest[texgyretermes-bold.otf]{texgyretermes-regular.otf}{\TeX{} Gyre Termes}
  \bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{ll}
  \toprule
  \engfonttest{Arial}{Arial}
  \engfonttest{Helvetica Neue}{Helvetica Neue}
  \engfonttest[texgyreheros-bold.otf]{texgyreheros-regular.otf}{\TeX{} Gyre Heros}
  \bottomrule
\end{tabular}
\end{center}

\subsection{Typewriter Fonts}
\ttfamily\upshape
\begin{center}
\ifgraduate%
  \small%
...
\begin{tabular}{ll}
  \toprule
  \engfonttest{Consolas}{Consolas}
  \engfonttest{Menlo}{Menlo}
  \engfonttest{Courier New}{Courier New}
  \engfonttest[texgyrecursor-bold.otf]{texgyrecursor-regular.otf}{\TeX{} Gyre Cursor}
  \bottomrule
\end{tabular}
\end{center}
...
\newcommand\mathtest{%
\begin{equation}%
  F(x; \, μ, \, σ) = \frac{1}{σ \sqrt{2π}} ∫_{-∞}^x \exp \left[ -\frac{(t-μ)^2}{2σ^2} \right] \, \symup{d} t%
\end{equation}%
}

\subsection{Latin Modern Math}
...
\subsection{\TeX{} Gyre Termes Math}
\setmathfont{texgyretermes-math.otf}[version=tgtermes]
\begingroup
  \mathversion{tgtermes}
  \mathtest
\endgroup

\subsection{STIX Math}
\setmathfont{STIXMath-Regular.otf}[version=STIX]
\begingroup
  \mathversion{STIX}
  \mathtest
\endgroup

\subsection{STIX Two Math}
\setmathfont{STIX2Math.otf}[StylisticSet=8, version=STIX2]
\begingroup
  \mathversion{STIX2}
  \mathtest
\endgroup

\subsection{XITS Math}
\setmathfont{XITSMath-Regular.otf}[StylisticSet=8, version=XITS]
\begingroup
  \mathversion{XITS}
  \mathtest
\endgroup
